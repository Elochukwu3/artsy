    ...
    <div className="px-20 max-lg:px-8 max-sm:px-6 my-20 ">
      <section>
        ...
        <div className="flex w-full items-center gap-6 my-10 max-lg:gap-4 max-md:flex-col  max-md:items-center  max-md:w-full relative">
          <>
            <FeaturedImage each_image={"/featured-img/feat-one.png"} headerText={"The Boolean Egyptian"} />
          </>
          <div className="flex-1 w-full h-full">
            <FeaturedText headerText={"The Boolean Egyptian"} />
          </div>
        </div>
      </section>
      <section>
        ...
        <div className="flex w-full items-center gap-6 my-10 max-lg:gap-4 max-md:flex-col-reverse max-md:flex-reverse max-md:items-center  max-md:w-full">
          <div className="flex-1 w-full h-full">
            <FeaturedText headerText={"are we there yet"} />
          </div>
          <>
            <FeaturedImage each_image={"/featured-img/feat-two.png"} headerText={"are we there yet"} />
          </>
        </div>
      </section>
      <section>
        ...
        <div className="flex w-full items-center gap-6 my-10 max-lg:gap-4 max-md:flex-col  max-md:items-center  max-md:w-full">
          <>
            <FeaturedImage each_image={"/featured-img/feat-three.png"} headerText={"oloibiri 1997"}/>
          </>
          <div className="flex-1 w-full h-full">
            <FeaturedText headerText={"oloibiri 1997"} />
          </div>
        </div>
      </section>
    </div>
...
function FeaturedImage({ each_image, headerText }) {
  return (
    <div className="flex-1 relative group max-[500px]:pointer-events-none">
      ...
      <div className="absolute
       inset-0 flex items-center justify-center w-11/12 gap-5
        ...
        <h2 className={`capitalize   hidden max-[500px]:text-2xl max-[500px]:text-[white]  max-[500px]:block font-semibold max-[330px]:text-[20px] text-center `}>
        {headerText}
